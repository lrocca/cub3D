...
/*   Updated: 2021/03/29 19:52:52 by lrocca           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		draw_column(int x)
{
	...
	if (g_cub.ray.x < 0 && (g_cub.ray.stepx = -1))
		g_cub.ray.sidedistx = (g_cub.plr.posx - g_cub.ray.mapx) \
			* g_cub.ray.deltadistx;
	else if ((g_cub.ray.stepx = 1))
		g_cub.ray.sidedistx = (g_cub.ray.mapx + 1.0 - g_cub.plr.posx) \
			* g_cub.ray.deltadistx;
	if (g_cub.ray.y < 0 && (g_cub.ray.stepy = -1))
		g_cub.ray.sidedisty = (g_cub.plr.posy - g_cub.ray.mapy) \
			* g_cub.ray.deltadisty;
	else if ((g_cub.ray.stepy = 1))
		g_cub.ray.sidedisty = (g_cub.ray.mapy + 1.0 - g_cub.plr.posy) \
			* g_cub.ray.deltadisty;
	side = perform_dda();
	before_print(side);
	put_column(x);
